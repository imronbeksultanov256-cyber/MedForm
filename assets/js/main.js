...
(function () {
  ...
  function initProcedurePage() {
    ...
    if (media) {
      if (p.img) {
        var mr = media.parentNode;
        if (mr) mr.innerHTML = '<img src="' + esc(p.img) + '" alt="' + esc(p.title) + '" loading="lazy" decoding="async">';
      } else if (p.ph) {
        media.classList.add(p.ph);
      }
    }
    ...
    if (rel) {
      rel.innerHTML = others.map(function (o, i) {
        return '' +
          ...
          '<div class="card__media"><div class="ratio r-4-5">' + (o.img ? '<img src="' + esc(o.img) + '" alt="' + esc(o.title) + '" loading="lazy" decoding="async">' : '<div class="ph ' + (o.ph || '') + '"></div>') + '</div></div>' +
